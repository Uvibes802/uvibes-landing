...
export const metadata: Metadata = buildMetadata("solution", "ru");
...
const breadcrumbJsonLd = {
  ...
  itemListElement: [
    ...
    { "@type": "ListItem", position: 2, name: "Метод", item: `${SITE_URL}/ru/method` },
  ],
};
...
export default function MethodPageRu() {
  return (
    <>
      ...
      <SolutionAnchorNav locale="ru" />

      <main className="sol-main-content">
        <SolutionHowItWorks locale="ru" />
        <SolutionThemes locale="ru" />
        ...
        <div className="str-gradient-wrap" style={{ background: HERO_GRADIENT, position: "relative", overflow: "hidden" }}>
          ...
          <SolutionStrategie locale="ru" />
          <WaveSeparator position="bottom" color="#FFF0F8" />
        </div>

        <SolutionSoftSkills locale="ru" />
        {/* Renvoi vers les tarifs, juste avant la section Résultats */}
        <div className="sol-offers-cta">
          <Link href="/ru/pricing" className="btn-brand sol-offers-cta-btn">
            Наши тарифы Uvibes →
          </Link>
        </div>
        <FeaturesCard locale="ru" />
        ...
        <div style={{ background: "linear-gradient(160deg, #FD6E00 0%, #FF6030 18%, #FF6098 45%, #E6007E 70%, #D90A5C 100%)", position: "relative", overflow: "hidden" }}>
          ...
          <section className="sol-tarifs-bridge">
            <p className="sol-bridge-eyebrow">Готовы начать?</p>
            <h2 className="sol-bridge-title v-prompt">Откройте для себя наши тарифы Uvibes</h2>
            <p className="sol-bridge-sub">
              Тарифы для любого коллектива, а также 30-дневное пробное предложение, чтобы попробовать без обязательств.
            </p>
            <Link href="/ru/pricing" className="btn-brand sol-bridge-cta">
