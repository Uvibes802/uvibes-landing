...
export const metadata: Metadata = buildMetadata("solution", "de");
...
const breadcrumbJsonLd = {
  ...
  itemListElement: [
    ...
    { "@type": "ListItem", position: 2, name: "Methode", item: `${SITE_URL}/de/method` },
  ],
};
...
export default function MethodPageDe() {
  return (
    <>
      ...
      <SolutionAnchorNav locale="de" />

      <main className="sol-main-content">
        <SolutionHowItWorks locale="de" />
        <SolutionThemes locale="de" />
        ...
        <div className="str-gradient-wrap" style={{ background: HERO_GRADIENT, position: "relative", overflow: "hidden" }}>
          ...
          <SolutionStrategie locale="de" />
          <WaveSeparator position="bottom" color="#FFF0F8" />
        </div>

        <SolutionSoftSkills locale="de" />
        {/* Renvoi vers les tarifs, juste avant la section Résultats */}
        <div className="sol-offers-cta">
          <Link href="/de/pricing" className="btn-brand sol-offers-cta-btn">
            Unsere Uvibes-Pläne →
          </Link>
        </div>
        <FeaturesCard locale="de" />
        ...
        <div style={{ background: "linear-gradient(160deg, #FD6E00 0%, #FF6030 18%, #FF6098 45%, #E6007E 70%, #D90A5C 100%)", position: "relative", overflow: "hidden" }}>
          ...
          <section className="sol-tarifs-bridge">
            <p className="sol-bridge-eyebrow">Bereit anzufangen?</p>
            <h2 className="sol-bridge-title v-prompt">Entdecken Sie unsere Uvibes-Pläne</h2>
            <p className="sol-bridge-sub">
              Pläne für jedes Kollektiv, plus ein 30-tägiges Testangebot, um es unverbindlich auszuprobieren.
            </p>
            <Link href="/de/pricing" className="btn-brand sol-bridge-cta">
