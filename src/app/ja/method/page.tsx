...
export const metadata: Metadata = buildMetadata("solution", "ja");
...
const breadcrumbJsonLd = {
  ...
  itemListElement: [
    ...
    { "@type": "ListItem", position: 2, name: "メソッド", item: `${SITE_URL}/ja/method` },
  ],
};
...
export default function MethodPageJa() {
  return (
    <>
      ...
      <SolutionAnchorNav locale="ja" />

      <main className="sol-main-content">
        <SolutionHowItWorks locale="ja" />
        <SolutionThemes locale="ja" />
        ...
        <div className="str-gradient-wrap" style={{ background: HERO_GRADIENT, position: "relative", overflow: "hidden" }}>
          ...
          <SolutionStrategie locale="ja" />
          <WaveSeparator position="bottom" color="#FFF0F8" />
        </div>

        <SolutionSoftSkills locale="ja" />
        {/* Renvoi vers les tarifs, juste avant la section Résultats */}
        <div className="sol-offers-cta">
          <Link href="/ja/pricing" className="btn-brand sol-offers-cta-btn">
            Uvibesプランを見る →
          </Link>
        </div>
        <FeaturesCard locale="ja" />
        ...
        <div style={{ background: "linear-gradient(160deg, #FD6E00 0%, #FF6030 18%, #FF6098 45%, #E6007E 70%, #D90A5C 100%)", position: "relative", overflow: "hidden" }}>
          ...
          <section className="sol-tarifs-bridge">
            <p className="sol-bridge-eyebrow">始める準備はできましたか？</p>
            <h2 className="sol-bridge-title v-prompt">Uvibesプランを発見する</h2>
            <p className="sol-bridge-sub">
              あらゆるコレクティフに対応したプランと、無料で試せる30日間のお試しプランをご用意。
            </p>
            <Link href="/ja/pricing" className="btn-brand sol-bridge-cta">
              プランと料金を見る →
            </Link>
          </section>
          <Footer locale="ja" />
