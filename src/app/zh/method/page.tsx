...
export const metadata: Metadata = buildMetadata("solution", "zh");
...
const breadcrumbJsonLd = {
  ...
  itemListElement: [
    ...
    { "@type": "ListItem", position: 2, name: "方法", item: `${SITE_URL}/zh/method` },
  ],
};
...
export default function MethodPageZh() {
  return (
    <>
      ...
      <SolutionAnchorNav locale="zh" />

      <main className="sol-main-content">
        <SolutionHowItWorks locale="zh" />
        <SolutionThemes locale="zh" />
        ...
        <div className="str-gradient-wrap" style={{ background: HERO_GRADIENT, position: "relative", overflow: "hidden" }}>
          ...
          <SolutionStrategie locale="zh" />
          <WaveSeparator position="bottom" color="#FFF0F8" />
        </div>

        <SolutionSoftSkills locale="zh" />
        {/* Renvoi vers les tarifs, juste avant la section Résultats */}
        <div className="sol-offers-cta">
          <Link href="/zh/pricing" className="btn-brand sol-offers-cta-btn">
            了解我们的 Uvibes 方案 →
          </Link>
        </div>
        <FeaturesCard locale="zh" />
        ...
        <div style={{ background: "linear-gradient(160deg, #FD6E00 0%, #FF6030 18%, #FF6098 45%, #E6007E 70%, #D90A5C 100%)", position: "relative", overflow: "hidden" }}>
          ...
          <section className="sol-tarifs-bridge">
            <p className="sol-bridge-eyebrow">准备好开始了吗？</p>
            <h2 className="sol-bridge-title v-prompt">了解我们的 Uvibes 方案</h2>
            <p className="sol-bridge-sub">
              为每个集体量身定制的方案，外加30天免费试用，无任何承诺即可体验。
            </p>
            <Link href="/zh/pricing" className="btn-brand sol-bridge-cta">
              查看方案与价格 →
            </Link>
          </section>
          <Footer locale="zh" />
